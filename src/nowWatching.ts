...
module.exports = async function nowWatching(firestore: any, {
  id,
  year = 2020,
  month,
  day
  ...
  const viewDeltas = _.tail(latestLogs.map((d: any, i: any, arr: any) => ({
    ...d,
    delta: i ? d.viewCount - arr[i - 1].viewCount : 0
  })))
  const smoothDeltas = gaussianSmoothing(viewDeltas.map((d: any) => d.delta), 3.5).map(d => d / minutes)
